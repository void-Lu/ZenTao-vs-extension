...
- **需求列表为空或部分缺失**：插件会合并项目需求、产品需求和执行需求列表，并按需求 ID 去重；如果项目需求接口返回空字段，产品或执行需求中的非空字段会补齐展示信息。若这些列表接口受权限限制或未返回需求，插件会继续从执行任务里的需求 ID 调用 `/stories/{id}` 补列表；仍缺失时可打开请求日志查看具体失败接口。
- **详情页显示"无法获取详情"**：需求详情按需求 ID 单独请求 `/stories/{id}`，任务详情按任务 ID 单独请求 `/tasks/{id}`。如果详情 API 返回 403 或其他错误，插件会优先使用本地缓存数据展示详情；如果本地也无缓存，会显示具体的错误信息，方便排查权限问题。
