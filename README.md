...
- **需求列表为空或部分缺失**：部分需求可能未关联执行，插件会尝试通过项目级需求接口补充加载；如果项目需求接口也返回 403，则仅显示通过产品获取到的需求。
- **详情页显示"无法获取详情"**：如果需求/任务详情 API 返回 403 或其他错误，插件会优先使用本地缓存数据展示详情；如果本地也无缓存，会显示具体的错误信息（如 403 状态码和服务端返回内容），方便排查权限问题。
